...
DB_FILE="egx_weekly_database_v2.json";REPORT_FILE="egx_weekly_database_quality_v2.json";TEMP_DB_FILE=DB_FILE+".tmp"
INITIAL_REFRESH_BARS=300;GAP_REFRESH_BARS=80;MAX_DATABASE_BARS=300;GAP_REFRESH_PERCENT=5.0;MAX_RETRIES=3;RETRY_DELAY=5;REQUEST_DELAY=2
...
def tv_bulk():
 for attempt in range(1,MAX_RETRIES+1):
  try:
   ...
   x=get_multiple_analysis(screener="egypt",interval=TVInterval.INTERVAL_1_WEEK,symbols=[f"EGX:{s}" for s in SYMBOLS])
   if x:return x
...
def quality(db,gaps,hist_ok,hist_fail,check_date):
 ...
 for name in SYMBOLS:
  ...
  if not df.empty:
   ...
   large=int((df["Close"].pct_change().abs()>=.10).sum());date_gaps=int((df.index.to_series().diff().dt.days>14).sum())
  report[name]={"bars":len(df),"start":str(df.index.min().date()) if len(df) else None,"end":str(df.index.max().date()) if len(df) else None,"bad_ohlc":bad,"large_moves_10pct":large,"date_gaps_over_14d":date_gaps,"automatic_gap_trigger":name in gaps,"historical_refresh":"success" if name in hist_ok else "failed" if name in hist_fail else "not_requested"}
...
def main():
 print("⚙️ EGX ENGINE v13.1 - TradingView Weekly Production Engine")
 print(f"📊 Final weekly database: {MAX_DATABASE_BARS} bars | Gap refresh: {GAP_REFRESH_BARS} bars | Manual/Initial: {INITIAL_REFRESH_BARS} bars")
 ...
 if not pulse and not MANUAL_REFRESH_HISTORY and not missing:
  ...
  return
 updated=[];gaps=[];auto_refresh_tickers=[];new_tickers=[];live_dates={}
 for name in SYMBOLS:
  ...
  if df.empty:
   auto_refresh_tickers.append(name);new_tickers.append(name);print(f"🆕 {name}: new symbol -> {INITIAL_REFRESH_BARS} bars")
  else:
 ...
 if MANUAL_REFRESH_HISTORY:targets=SYMBOLS;bars=INITIAL_REFRESH_BARS
  ...
  bars=INITIAL_REFRESH_BARS if new_tickers else GAP_REFRESH_BARS
