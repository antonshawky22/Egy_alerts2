...
# ==========================================
# EGX LADDER CYCLE SYSTEM - FULL BACKTEST v1.3
...
def create_trade(symbol,signal_date,execution_date,price,market_df):
    return {
        "symbol":symbol,
        "status":"OPEN",
        "cycle":1,
        "signal_date":str(pd.Timestamp(signal_date).date()),
        "first_entry_date":str(pd.Timestamp(execution_date).date()),
        "last_entry_date":str(pd.Timestamp(execution_date).date()),
        "exit_date":None,
        ...
        "entry_market_regime":get_market_regime(execution_date,market_df),
        "entry_egx30_close":get_market_value(execution_date,market_df,"close"),
        "entry_egx30_ema95":get_market_value(execution_date,market_df,"ema95"),
...
def close_trade(state,signal_date,execution_date,price,reason,market_df):
    ...
    state["exit_signal_date"]=str(pd.Timestamp(signal_date).date())
    state["exit_date"]=str(pd.Timestamp(execution_date).date())
    state["exit_reason"]=reason
    state["exit_market_regime"]=get_market_regime(execution_date,market_df)
    state["holding_days"]=(pd.Timestamp(execution_date)-pd.Timestamp(state["first_entry_date"])).days
    state["status"]="CLOSED"
    state["position"]=0
    return state

def partial_sell(state,signal_date,execution_date,price,sell_position,reason):
    profit=calculate_current_profit(state,price)
    ...
    state["last_sell_signal_date"]=str(pd.Timestamp(signal_date).date())
    state["last_sell_date"]=str(pd.Timestamp(execution_date).date())
    state["exit_reason"]=reason
    return state

def ladder_stage(state):
    pos=state["position"]
    if state["third_entry"] is not None:
        return "L3"
    if state["second_entry"] is not None:
        return "L2"
    return "L1"
...
def run_symbol_backtest(symbol,raw_database,market_df):
    ...
    # Last usable signal day is len(df)-2 because execution is next day's OPEN.
    for i in range(len(df)-1):
        signal_date=df.index[i]
        execution_date=df.index[i+1]

        row=df.iloc[i]
        execution_row=df.iloc[i+1]
        ...
        # ------------------------------------------
        # UPDATE OPEN TRADE USING CURRENT CLOSE
        # ------------------------------------------
        if state is not None:
            state["last_price"]=round(signal_close,6)
            current_profit=calculate_current_profit(state,signal_close)
        ...
        if state is None:
            if (
                ...
                signals.append({
                    ...
                    "action":"BUY L1",
                    "signal_price":round(signal_close,6),
                    "execution_price":round(execution_open,6),
        ...
        # ------------------------------------------
        # EXIT / SELL SIGNALS
        # Signal from today's CLOSE
        # Execute tomorrow at OPEN
        # ------------------------------------------
        pos=state["position"]
        avg=state["avg_price"]
        profit=calculate_current_profit(state,signal_close)
        ...
        trailing=(
            ...
            and state["peak_profit"]-profit>=TRAILING_GIVEBACK
        )
        ...
        if state is not None:
            ...
            if (
                ...
                state["second_entry"]={
                    ...
                    "price":round(execution_open,6)
                }

                state["last_entry_date"]=str(execution_date.date())
    ...
    if state is not None:
        ...
        state["profit_pct"]=round(
            ...
            calculate_current_profit(state,end_price)*state["position"],
            4
        )

        state["exit_market_regime"]=get_market_regime(
            end_date,
            market_df
        )
...
def ladder_stage_summary(trades):
    closed=[t for t in trades if t.get("status")=="CLOSED"]
    ...
    for t in closed:
        if t.get("third_entry") is not None:
            reached_l3+=1
        elif t.get("second_entry") is not None:
            reached_l2+=1
        else:
            l1_only+=1

    return {
        "l1_only_trades":l1_only,
        "reached_l2_trades":reached_l2,
        "reached_l3_trades":reached_l3
    }
...
def regime_trade_stats(trades,regime):
    selected=[
        ...
        and t.get("entry_market_regime")==regime
    ]

    stats=calculate_trade_stats(selected)

    stats["stop_loss_count"]=sum(
        t.get("exit_reason")=="STOP LOSS"
        for t in selected
    )

    stats["trailing_stop_count"]=sum(
        t.get("exit_reason")=="TRAILING STOP"
        for t in selected
    )

    stats["l3_count"]=sum(
        t.get("third_entry") is not None
        for t in selected
    )
...
def portfolio_simulation(closed_trades):
    ...
    return {
        ...
        "equity_curve":equity,
        "portfolio_model":"Simple sequential model at 12.5% per closed trade."
    }
...
def market_summary(market_df):
    ...
    for regime in [
        ...
        count=int(
            (market_df["market_regime"]==regime).sum()
        )
    ...
    below=int(
        market_df["below_ema95"].sum()
    )

    above=int(
        market_df["above_ema95"].sum()
    )

    below_streak=longest_true_streak(
        market_df["below_ema95"]
        .fillna(False)
        .tolist()
    )

    bear_streak=longest_true_streak(
        (
            market_df["market_regime"]=="BEAR"
        ).tolist()
    )

    return {
        "period_start":str(
            market_df.index.min().date()
        ),
        "period_end":str(
            market_df.index.max().date()
        ),
        "total_days":total,
        "regimes":result,
        "simple_above_ema95_days":above,
        "simple_below_ema95_days":below,
        "simple_above_ema95_percent":round(
            above/total*100,2
        ),
        "simple_below_ema95_percent":round(
            below/total*100,2
        ),
...
print(
    f"📅 EGX30 PERIOD: "
    f"{market_df.index.min().date()} -> "
    f"{market_df.index.max().date()}"
)
...
for n,symbol in enumerate(SYMBOLS,1):
    ...
    trades,signals=run_symbol_backtest(
        symbol,
        raw_database,
        market_df
    )
...
ladder_summary=ladder_stage_summary(
    all_trades
)
...
regime_stats={
    r:regime_trade_stats(all_trades,r)
    for r in regimes
}
...
for t in open_trades:
    ...
    open_by_regime[r]=(
        open_by_regime.get(r,0)+1
    )

transition_stats=regime_transition_stats(
    all_trades
)
...
for symbol in SYMBOLS:
    ...
    ladder=ladder_stage_summary(st)
...
print(
    f"Final Capital       : "
    f"{portfolio['final_capital']:,.2f}"
)

print(
    f"Compound Return     : "
    f"{portfolio['compound_return_percent']:.2f}%"
)

print(
    f"Max Drawdown        : "
    f"{portfolio['max_drawdown_percent']:.2f}%"
)

print(
    "⚠️ "+portfolio["portfolio_model"]
)

print("\n"+"="*70)
print("🪜 LADDER DEPTH ANALYSIS")
print("="*70)

print(
    f"L1 Only             : "
    f"{ladder_summary['l1_only_trades']}"
)

print(
    f"Reached L2          : "
    f"{ladder_summary['reached_l2_trades']}"
)

print(
    f"Reached L3          : "
    f"{ladder_summary['reached_l3_trades']}"
)
...
print(
    f"Above EMA95         : "
    f"{mkt['simple_above_ema95_days']} days "
    f"({mkt['simple_above_ema95_percent']}%)"
)

print(
    f"Below EMA95         : "
    f"{mkt['simple_below_ema95_days']} days "
    f"({mkt['simple_below_ema95_percent']}%)"
)

print(
    f"Longest Below EMA95 : "
    f"{mkt['longest_below_ema95_streak']} days"
)

print(
    f"Longest BEAR        : "
    f"{mkt['longest_bear_regime_streak']} days"
)
...
if regime_stats["BEAR"]["trades"]==0:
    ...
    print(
        "⚠️ THIS BACKTEST CANNOT VALIDATE "
...
print(
    f"Stop Loss            : "
    f"{exit_counts.get('STOP LOSS',0)}"
)

print(
    f"Trailing Stop        : "
    f"{exit_counts.get('TRAILING STOP',0)}"
)

print(
    f"Full Exit            : "
    f"{exit_counts.get('FULL EXIT',0)}"
)

print(
    f"Max Concurrent Trades: "
    f"{max_concurrent_trades(all_trades)}"
)

print(
    f"Max Consecutive Loss : "
    f"{consecutive_losses(all_trades)}"
)
...
if holding:
    print(
        f"Longest Holding Days : "
        f"{max(holding)}"
    )

    print(
        f"Average Holding Days : "
        f"{round(np.mean(holding),1)}"
    )
...
results={
    "backtest_version":"v1.3",
    "execution_model":"Signal at CLOSE -> execution at NEXT DAY OPEN",
    ...
    "ladder_depth_summary":ladder_summary,
    ...
    "max_concurrent_trades":
        max_concurrent_trades(all_trades),

    "max_consecutive_losses":
        consecutive_losses(all_trades),

    "longest_holding_days":
        max(holding) if holding else 0,

    "average_holding_days":
        round(float(np.mean(holding)),2)
        if holding else 0,

    "open_trades":len(open_trades),
    "closed_trades":len(closed_trades),

    "portfolio_model_warning":
        "Simple sequential 12.5% per closed trade model; "
        "not a true concurrent portfolio."
}
