...
INITIAL_REFRESH_BARS=500;GAP_REFRESH_BARS=250;MAX_DATABASE_BARS=500;GAP_REFRESH_PERCENT=5.0;MAX_RETRIES=3;RETRY_DELAY=5;REQUEST_DELAY=2
...
def bulk_row(bulk,name):
 try:
  ...
  i=x.indicators
  return {c:float(i[c]) for c in COLUMNS}
 except:return None
def valid_row(r):
 try:
...
def quality(db,gaps,hist_ok,hist_fail,check_date):
 ...
 for name in SYMBOLS:
  ...
  if not df.empty:
   ...
   moves=df["Close"].pct_change().abs();large=int((moves>=.10).sum())
   date_gaps=int((df.index.to_series().diff().dt.days>5).sum())
...
def main():
 print("⚙️ EGX ENGINE v13.0 - TradingView Production Engine")
 ...
 if not bulk:
  ...
  print("❌ TradingView bulk fetch failed. Database unchanged.");return
 pulse=False
 for n in ["COMI","SWDY","HRHO"]:
  r=bulk_row(bulk,n);df=db.get(n,pd.DataFrame())
  if r and (df.empty or len(df)==0 or not valid_row(r)):pulse=True;break
  if r and not df.empty:
   old=df.iloc[-1]
   if any(abs(float(r[c])-float(old[c]))>0.000001 for c in ["Open","High","Low","Close","Volume"]):pulse=True
 if not pulse and not MANUAL_REFRESH_HISTORY:
  print("ℹ️ No real market activity detected. Safe exit.")
  return
 check_date=pd.Timestamp.now().normalize();updated=[];gaps=[];auto_refresh_tickers=[]
 for name in SYMBOLS:
  r=bulk_row(bulk,name)
  if not r or not valid_row(r):
  ...
  df.loc[check_date,COLUMNS]=[r[c] for c in COLUMNS];db[name]=normalize(df);updated.append(name)
 if MANUAL_REFRESH_HISTORY:
  targets=SYMBOLS;bars=INITIAL_REFRESH_BARS
 elif auto_refresh_tickers:
  targets=auto_refresh_tickers;bars=GAP_REFRESH_BARS
 else:targets=[];bars=GAP_REFRESH_BARS
 hist_ok=[];hist_fail=[]
 if targets:
  if not HISTORICAL_API_AVAILABLE:
   hist_fail=targets.copy();print("❌ Historical API unavailable")
  else:
   print(f"🔄 Historical refresh: {len(targets)} symbols × {bars} bars")
   for name in targets:
    print(f"📥 {name}...")
    h=historical(name,bars)
    if h.empty:
     hist_fail.append(name);print(f"❌ {name}: refresh failed");continue
    df=db.get(name,pd.DataFrame(columns=COLUMNS)).copy()
    if df.empty:df=h
    else:
     df=pd.concat([df,h]);df=df[~df.index.duplicated(keep="last")].sort_index()
    df=normalize(df)
    if name in updated and check_date in df.index:
     r=bulk_row(bulk,name)
     if r:df.loc[check_date,COLUMNS]=[r[c] for c in COLUMNS];df=normalize(df)
    db[name]=df;hist_ok.append(name);print(f"✅ {name}: {len(df)} bars")
    time.sleep(REQUEST_DELAY)
 save_database(db);quality(db,[x[0] for x in gaps],hist_ok,hist_fail,check_date)
 print(f"💾 Database saved: {DB_FILE}")
 print(f"📊 Updated: {len(updated)}/{len(SYMBOLS)}")
 print(f"📈 Gap targets: {len(gaps)}")
 print(f"🔄 Historical success: {len(hist_ok)} | failed: {len(hist_fail)}")
 if MANUAL_REFRESH_HISTORY:
  msg=f"🔵 *Manual Historical Refresh*\n📅 Date: {check_date.date()}\n🔄 Bars: {INITIAL_REFRESH_BARS}\n✅ Success: {len(hist_ok)}/{len(SYMBOLS)}"
  if hist_fail:msg+=f"\n❌ Failed: {', '.join(hist_fail)}"
 elif gaps:
  msg=f"⚠️ *Gap Refresh Triggered*\n📅 Date: {check_date.date()}\n🔍 Threshold: ±{GAP_REFRESH_PERCENT}%\n🔄 Bars: {GAP_REFRESH_BARS}\n🎯 Targets: {', '.join(x[0] for x in gaps)}\n🔵 Historical Success: {len(hist_ok)}"
  if hist_fail:msg+=f"\n❌ Failed: {', '.join(hist_fail)}"
 else:
  msg=f"✅ *EGX Price Database Updated*\n📅 Date: {check_date.date()}\n📊 Updated: {len(updated)}/{len(SYMBOLS)}"
