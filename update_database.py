...
last_candle_date = "2026-06-18" # تاريخ آخر جلسة مغلقة (الخميس)
...
for name, ticker in symbols.items():
    try:
        ...
        if not stock_data or len(stock_data) < 100:
            ...
            yf_df = yf.download(f"{ticker}.CA", period="7mo", interval="1d", auto_adjust=False, progress=False)
        ...
        # دمج أو تحديث سطر الجلسة الحالية
        ...
        print(f"✅ {name} updated. Total days: {len(df)}")
...
print("🏁 Database update complete. File 'egx_history_database.json' is ready.")
